...
#define B2SHAPE(_ret_, _name_, ...) BOX2DCAPI _ret_ b2Shape##_name_(b2Shape* shape, ##__VA_ARGS__)

#define B2SHAPEDER(_ret_, _typename_, _type_, _name_, ...) BOX2DCAPI _ret_ b2Shape##_typename_##_##_name_(_type_* shape, ##__VA_ARGS__)
...
// b2CircleShape
B2SHAPEDER(void, Circle, b2CircleShape, SetPosition, const b2Vec2& p) {
    shape->m_p = p;
}

// b2EdgeShape
B2SHAPEDER(void, Edge, b2EdgeShape, SetOneSided, const b2Vec2& v0, const b2Vec2& v1, const b2Vec2& v2, const b2Vec2& v3) {
    shape->SetOneSided(v0, v1, v2, v3);
}

B2SHAPEDER(void, Edge, b2EdgeShape, SetTwoSided, const b2Vec2& v1, const b2Vec2& v2) {
    shape->SetTwoSided(v1, v2);
}

// b2PolygonShape
B2SHAPEDER(void, Polygon, b2PolygonShape, Set, const b2Vec2* points, int32 count) {
    shape->Set(points, count);
}

B2SHAPEDER(void, Polygon, b2PolygonShape, SetAsBox, float hx, float hy) {
    shape->SetAsBox(hx, hy);
}

B2SHAPEDER(void, Polygon, b2PolygonShape, SetAsBoxWithCenterAngle, float hx, float hy, const b2Vec2& center, float angle) {
    shape->SetAsBox(hx, hy, center, angle);
}

// b2ChainShape
B2SHAPEDER(void, Chain, b2ChainShape, Clear) {
    shape->Clear();
}

B2SHAPEDER(void, Chain, b2ChainShape, CreateLoop, const b2Vec2* vertices, int32 count) {
    shape->CreateLoop(vertices, count);
}

B2SHAPEDER(void, Chain, b2ChainShape, CreateChain, const b2Vec2* vertices, int32 count, const b2Vec2& prevVertex, const b2Vec2& nextVertex) {
